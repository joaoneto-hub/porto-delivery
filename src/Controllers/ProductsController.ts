...
class ProductsController {
  ...
  static async getAllById(req: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
  ...
  static async create(req: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      ...
      if (!name || !price || !userId) {
        reply.status(400).send({ error: 'Nome, preço e userId são obrigatórios.' });
        return;
      }
      ...
      reply.status(201).send({ message: 'Produto criado com sucesso.', productId });
    } catch (error) {
  ...
  static async update(req: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      ...
      if (!name && price === undefined && !description) {
        reply.status(400).send({ error: 'Forneça pelo menos um campo para atualização.' });
        return;
      }
